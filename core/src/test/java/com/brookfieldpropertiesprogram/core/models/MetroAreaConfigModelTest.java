...
class MetroAreaConfigModelTest {

    final AemContext context = BrookfieldTestContext.newAemContext();
    MetroAreaConfigModel toBeTested;

    @BeforeEach
    public void setup() throws Exception {
        Page currentPage = context.currentPage(BrookfieldTestContext.CONFIG_M);
        Resource currentResource = context.currentResource(BrookfieldTestContext.CONFIG_M);

        toBeTested = context.currentPage().adaptTo(MetroAreaConfigModel.class);
    }
    ...
    void testTitle() throws Exception {
        ...
        Assertions.assertEquals("Chicago Metro Area", toBeTested.getTitle());
    }
}
